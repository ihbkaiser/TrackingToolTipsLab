...
# Hàm chuyển (B, C, H, W) -> (B, N, C) với N = H*W
def image_to_graph(x):
    B, C, H, W = x.shape
    N = H * W
    return x.view(B, C, N).permute(0, 2, 1).to(device="cuda")

# Lập edge_index cho lưới pixel với 4-kề (dành cho lưới kích thước H x W)
def build_edge_index(H, W):
    edges = []
    for i in range(H):
        for j in range(W):
            idx = i * W + j
    ...
    return edge_index.to(device="cuda")
...
class BasicConv(nn.Module):
    def __init__(self, in_planes, out_planes, kernel_size, stride, padding, dilation=1, relu=True):
